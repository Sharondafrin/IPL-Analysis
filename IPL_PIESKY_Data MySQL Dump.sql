...
select stadium_name,city,count(ipm.match_id) as no_of_matches,ipms.stadium_id
from ipl_match ipm join ipl_match_schedule ipms
on ipm.match_id=ipms.match_id join ipl_stadium ips
on ipms.stadium_id=ips.stadium_id
group by ipms.stadium_id;
...
select ipt.team_name,ipd.bidder_id,bid_team,no_of_bids as total_bids
from IPL_Bidder_Details ipd join IPL_Bidding_Details ip
on ipd.bidder_id=ip.bidder_id join IPL_Bidder_Points ipb
on ipd.bidder_id=ipb.bidder_id join ipl_team ipt
where ipt.team_id=bid_team
group by ipt.team_name;
...
select ip.team_id,team_name,sum(matches_played) as total_matches_palyed,sum(Matches_won) as 
total_matches_won,sum(Matches_lost) as total_matches_lost
from ipl_team ip join ipl_team_standings ipt
on ip.team_id=ipt.team_id
group by team_name;

#7.	Display the bowlers for Mumbai Indians team.
select ip.team_id,ip.team_name,player_name from ipl_team ip
join ipl_team_players ipt
on ip.team_id=ipt.team_id
join ipl_player ipl on ipt.player_id=ipl.player_id
where team_name='Mumbai Indians' and player_role='bowler';

#8.	How many all-rounders are there in each team, Display the teams with more than 4 
#all-rounder in descending order.
select ip.team_id,ip.team_name,player_name,count(ipt.player_role) as all_rounders from 
ipl_team ip
join ipl_team_players ipt
on ip.team_id=ipt.team_id
join ipl_player ipl on ipt.player_id=ipl.player_id
where ipt.player_role like '%rounder%'
group by team_name
having count(ipt.player_role)>4 
order by  count(ipt.player_role) desc;
